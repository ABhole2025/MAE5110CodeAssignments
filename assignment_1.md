...
$\theta^+ = \theta^-+\gamma+2\alpha, \qquad \dot{\theta}^+ = \dot{\theta}^-\cos(2\alpha)$
...
The Floquet multiplier was estimated from the local slope of the return map:
...
the walking limit cycle is locally asymptotically stable. A perturbation from the periodic gait is therefore reduced from one step to the next, indicating that the simulated motion has reached the expected periodic walking gait.
...
$\dot{\theta}^{-\ast}=\frac{\dot{\theta}^{+\ast}}{\cos(45^\circ)}\approx-2.7060\text{ rad/s}$
